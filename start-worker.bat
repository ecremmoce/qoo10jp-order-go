...
powershell -Command "try { $response = Invoke-WebRequest -Uri 'http://localhost:8080/api/v1/health' -Method GET -TimeoutSec 5; Write-Host 'OK Application Running'; } catch { Write-Host 'ERROR Application not running. Please start qoo10jp-order.exe first.'; pause; exit; }"
